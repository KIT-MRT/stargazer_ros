    ...
    for (int j = 0; j < measurements.size(); j++) {
        ...
        for (int k = 0; k < shot.size(); k++) {
            ...
            for (int i = 0; i < lm_obs.points.size(); i++) {
                ...
                transformLM2Img<double>(&x_marker, &y_marker, lm.pose.data(), camera_pose.data(),
                                        camera_intrinsics.data(), &u_marker, &v_marker);

                auto pt_real = cvPoint(u_marker, v_marker);
                auto pt_obs =
                    cvPoint(std::get<(int)POINT::X>(lm_obs.points[i]), std::get<(int)POINT::Y>(lm_obs.points[i]));
